...
public sealed class UpsertBrandFaceProfileValidator : AbstractValidator<UpsertBrandFaceProfileCommand>
{
    public UpsertBrandFaceProfileValidator()
    {
        ...
        RuleFor(command => command.Telegram).NotEmpty().Matches("^@[A-Za-z0-9_]{5,32}$");
        RuleFor(command => command.Instagram).Matches("^@[A-Za-z0-9._]{1,30}$").When(command => command.Instagram is not null);
        ...
        RuleFor(command => command.PortfolioUrl).Must(value => Uri.TryCreate(value, UriKind.Absolute, out _)).When(command => command.PortfolioUrl is not null);
        RuleFor(command => command.AvatarUrl).Must(value => Uri.TryCreate(value, UriKind.Absolute, out _)).When(command => command.AvatarUrl is not null);
