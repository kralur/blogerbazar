...
export function ActiveFilterChips<Key extends string>({ chips, onRemove, onReset }: { chips: Array<{ key: Key; label: string }>; onRemove: (key: Key) => void; onReset: () => void }) {
  const { t } = useI18n();
  ...
  return <div aria-label={t("search.activeFilters")} className="catalog-search__chips" role="group">
    ...
    <button className="catalog-search__reset-all" onClick={onReset} type="button">{t("search.resetAll")}</button>
  </div>;
}
