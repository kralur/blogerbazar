...
export function BloggerDetails({ id }: { id: string }) {
  ...
  return <div className="screen pb-36">
    ...
    <section className="mt-5"><h2 className="mb-3 font-extrabold">{t("details.contacts")}</h2>{contactLine ? <Card><p className="text-sm font-bold">{contactLine}</p>{contact?.phone && contact.email && <p className="mt-1 text-sm text-brand-muted">{contact.email}</p>}</Card> : <Card><p className="text-sm text-brand-muted">{t("common.contactMissing")}</p></Card>}</section>
