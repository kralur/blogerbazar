...
export function BloggerSearch() {
  ...
  return <div className="screen screen--with-nav space-y-4 px-4 pt-5">
    <header className="flex items-center justify-between gap-3"><div><p className="text-sm font-semibold text-brand-muted">{t("search.eyebrow")}</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight">{t("search.title")}</h1></div><div className="flex shrink-0 items-center gap-2"><LanguageSwitcher /><button aria-expanded={filtersOpen} aria-label={t("search.filters")} className="grid h-11 w-11 place-items-center rounded-2xl bg-white shadow-card" onClick={() => setFiltersOpen((value) => !value)} type="button"><Icon name="filter" /></button></div></header>
