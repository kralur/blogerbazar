...
export function MyCampaigns() {
  ...
  const refreshAccess = useCallback(() => {
    if (!active) return;
    let cancelled = false;
    const preserveAllowedAccess = accessRef.current === "allowed";
    if (!preserveAllowedAccess) setAccess("checking");
    getCurrentPlatformUser().then((user) => {
      if (cancelled) return;
      const nextAccess = normalizeMarketplaceRole(user.selectedMarketplaceRole) === "Business" ? "allowed" : "denied";
      accessRef.current = nextAccess;
      setAccess(nextAccess);
    }).catch(() => {
      if (!cancelled && !preserveAllowedAccess) setAccess("failed");
    });
