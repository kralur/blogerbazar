...
function Rail({ title, empty, children }: { title: string; empty: string; children: ReactNode }) {
  const { t } = useI18n();
  return <section className="mt-7"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-extrabold">{title}</h2><a className="text-sm font-bold text-brand-blue" href="#/search">{t("home.viewAll")}</a></div>{children ? <div className="home-rail no-scrollbar -mx-4 flex gap-3 overflow-x-auto px-4 pb-2">{children}</div> : <Card><p className="text-sm text-brand-muted">{empty}</p></Card>}</section>;
}
...
export function Home() {
  ...
  return <div className="home-showcase screen screen--with-nav">
    ...
    {failed ? <div className="mt-5"><ErrorState onRetry={load} subtitle={t("home.loadFailed")} title={t("home.loadFailed")} /></div> : !data ? <HomeSkeleton /> : <>
      ...
      <Rail empty={t("home.topRatedEmpty")} title={t("home.topRated")}>{data.topRatedBloggers.length ? data.topRatedBloggers.map(blogger => <div className="home-rail__blogger shrink-0 snap-start" key={blogger.id}><BloggerCard blogger={blogger} /></div>) : null}</Rail>
      <Rail empty={t("home.newBloggersEmpty")} title={t("home.newBloggers")}>{data.newBloggers.length ? data.newBloggers.map(blogger => <div className="home-rail__blogger shrink-0 snap-start" key={blogger.id}><BloggerCard blogger={blogger} /></div>) : null}</Rail>
      <section className="mt-7"><h2 className="mb-3 text-lg font-extrabold">{t("onboarding.brandFace")}</h2>{data.newBrandFaces.length ? <div className="home-rail no-scrollbar -mx-4 flex gap-3 overflow-x-auto px-4 pb-2">{data.newBrandFaces.map(profile => <a aria-label={profile.name} className="glass-card w-[236px] shrink-0 snap-start p-4" href={`#/brand-face-detail/${profile.id}`} key={profile.id}><div className="flex items-center gap-3"><Avatar name={profile.name} size="sm" src={profile.avatarUrl} /><div className="min-w-0"><h3 className="truncate font-extrabold">{profile.name}</h3><p className="mt-1 text-xs text-brand-muted">{cityLabel(profile.city, language)}</p></div></div><div className="mt-3 flex flex-wrap gap-1">{profile.categories.slice(0, 2).map(category => <Chip key={category}>{categoryLabel(category, language)}</Chip>)}</div><p className="mt-3 line-clamp-2 text-sm text-brand-muted">{profile.description || profile.experience || t("common.noData")}</p><p className="mt-4 text-sm font-extrabold">{profile.collaborationPrice ? formatCurrency(profile.collaborationPrice) : t("card.onRequest")}</p></a>)}</div> : <Card><p className="text-sm text-brand-muted">{t("common.noData")}</p></Card>}</section>
      <section className="mt-7"><h2 className="text-lg font-extrabold">{t("home.categories")}</h2><div className="mt-3 flex flex-wrap gap-2">{data.categories.map(category => <a href={`#/search?category=${encodeURIComponent(category)}`} key={category}><Chip>{categoryLabel(category)}</Chip></a>)}</div></section>
      <section className="mt-7"><h2 className="mb-3 text-lg font-extrabold">{t("home.statistics")}</h2><div className="grid grid-cols-2 gap-2"><StatsCard icon="users" label={t("home.approvedBloggers")} value={formatNumber(data.statistics.approvedBloggers)} /><StatsCard icon="building" label={t("common.companies")} value={formatNumber(data.statistics.companies)} /><StatsCard icon="briefcase" label={t("home.activeCampaigns")} value={formatNumber(data.statistics.activeCampaigns)} /><StatsCard icon="star" label={t("common.rating")} value={data.statistics.averageRating?.toFixed(1) ?? "—"} /></div></section>
    </>}
