...
export function BloggerProfileForm({ onCompleted, onBackToRole }: { onCompleted?: () => void; onBackToRole?: () => void }) {
  ...
  return <form className={`screen ${onCompleted ? "screen--without-nav" : "screen--with-nav"} space-y-5 px-4 pt-5`} noValidate onSubmit={submit}>
    <header data-content-header><p className="text-sm font-semibold text-brand-muted">{t("form.blogger.eyebrow")}</p><h1 className="mt-1 text-3xl font-extrabold tracking-tight">{existing ? t("form.blogger.editTitle") : t("form.blogger.createTitle")}</h1><p className="mt-2 text-sm leading-5 text-brand-muted">{t("form.blogger.subtitle")}</p></header>
