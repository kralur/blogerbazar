...
export function TelegramAuthorization({ isTelegram, loading, failed, onContinue }: { isTelegram: boolean; loading: boolean; failed: boolean; onContinue: () => void }) {
  ...
  return <main className="screen flex min-h-screen flex-col px-5 pb-8 pt-10">
    ...
    <Button aria-busy={loading} className="mt-auto w-full" disabled={loading} onClick={onContinue} type="button"><Icon name="send" />{loading ? t("firstRun.authorizing") : action}</Button>
  </main>;
}
